...
const TITLES: Record<RoadmapPeriod, string> = {
  ...
  'jun': 'AI 즉시 매칭 추천 + 이상 징후 감지 + 안심결제 전국 도입',
};
...
const SUBTITLES: Record<RoadmapPeriod, string> = {
  ...
  'jun': 'AI 즉시 매칭 · 이상 징후 감지 · 지역 기반 추천 · 안심결제 전국 확대',
};
...
const DESCRIPTIONS: Record<RoadmapPeriod, string> = {
  ...
  'jun': '판단 보조 시스템 완성. 공고 등록 즉시 AI 매칭 추천(추천 이유 카드), 이상 징후 감지 v1(미연락·결제지연·저만족도), 지역 기반 추천 고도화, 안심결제 전국 확대 + 요금 역전 해소.',
};
